...
void power_init(int mode)
{
	...
	serial_put_dec(AML_VCCK_INIT_VOLTAGE);
	serial_puts(" mv\n");
	pwm_set_voltage(pwm_ao_b, AML_VCCK_INIT_VOLTAGE);
	serial_puts("set vddee to ");
	serial_put_dec(AML_VDDEE_INIT_VOLTAGE);
	serial_puts(" mv\n");
	pwm_set_voltage(pwm_ao_d, AML_VDDEE_INIT_VOLTAGE);
	pwm_init(pwm_ao_b);
	pwm_init(pwm_ao_d);
